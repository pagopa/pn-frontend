...
function i18nLabelEntry(category, deliveryDetailCode) {
...
export class SendAnalogFlowStep extends TimelineStep {
    getTimelineStepLabel(payload) {
        const typedDetails = payload.step.details;
        ...
        return getLocalizedOrDefaultLabel('notifications', `detail.timeline.${labelEntry}`, defaultLabel);
    }
    getTimelineStepInfo(payload) {
        ...
        const originatingStep = payload.allStepsForThisStatus?.find((step) => step.elementId === payload.step.details.sendRequestId);
        const registeredLetterKindCode = originatingStep && originatingStep.details.productType;
        ...
        const deliveryFailureCauseCode = payload.step.details
        ...
        const deliveryFailureCauseText = deliveryFailureCauseCode
            ...
            : '';
        const registeredLetterNumber = payload.step.details.registeredLetterCode ?? '';
        const physicalAddress = originatingStep
            ? this.completePhysicalAddressFromStep(originatingStep)
            : {};
        // eslint-disable-next-line functional/no-let
        let description = getLocalizedOrDefaultLabel('notifications', `detail.timeline.send-analog-flow-${deliveryDetailCode}-description${payload.isMultiRecipient ? '-multirecipient' : ''}`, '', {
        ...
        if (description.length === 0) {
            description = getLocalizedOrDefaultLabel('notifications', `detail.timeline.send-analog-flow-description`, `C'è un aggiornamento sull'invio cartaceo.`);
        }
