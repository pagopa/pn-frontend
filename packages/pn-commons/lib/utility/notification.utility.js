...
import { AppIoCourtesyMessageEventType, DigitalDomicileType, LegalFactType, NotificationDeliveryMode, NotificationStatus, PaymentStatus, TimelineCategory, } from '../models';
 ...
 */
function localizeStatus(status, defaultLabel, defaultTooltip, defaultDescription, data) {
    const isMultiRecipient = data?.isMultiRecipient;
    // eslint-disable-next-line functional/no-let
    let filteredData = _.omit(data, ['isMultiRecipient']);
    if (Object.keys(filteredData).length === 0) {
        filteredData = undefined;
    }
    return {
        label: getLocalizedOrDefaultLabel('notifications', `status.${status}${isMultiRecipient ? '-multirecipient' : ''}`, defaultLabel),
        tooltip: getLocalizedOrDefaultLabel('notifications', `status.${status}-tooltip${isMultiRecipient ? '-multirecipient' : ''}`, defaultTooltip, filteredData),
        description: getLocalizedOrDefaultLabel('notifications', `status.${status}-description${isMultiRecipient ? '-multirecipient' : ''}`, defaultDescription, filteredData),
    };
}
/**
 ...
 * @returns object
 */
export function getNotificationStatusInfos(status, options) {
    const statusComesAsAnObject = !!status.status;
    const statusObject = statusComesAsAnObject
        ? status
        : undefined;
    const actualStatus = statusComesAsAnObject
        ? status.status
        : status;
    ...
    switch (actualStatus) {
        case NotificationStatus.DELIVERED:
            const statusInfos = localizeStatus('delivered', 'Consegnata', `La notifica è stata consegnata`, 'La notifica è stata consegnata.', { isMultiRecipient });
            ...
            if (deliveryMode && !isMultiRecipient) {
                const deliveryModeDescription = getLocalizedOrDefaultLabel('notifications', `status.deliveryMode.${deliveryMode}`, `${deliveryMode}`);
                statusInfos.description = getLocalizedOrDefaultLabel('notifications', 'status.delivered-description-with-delivery-mode', `La notifica è stata consegnata per via ${deliveryMode === NotificationDeliveryMode.ANALOG ? 'analogica' : 'digitale'}.`, { deliveryMode: deliveryModeDescription });
            }
        ...
        case NotificationStatus.DELIVERING:
            return {
                color: 'default',
                ...localizeStatus('delivering', 'Invio in corso', "L'invio della notifica è in corso", "L'invio della notifica è in corso"),
            };
        case NotificationStatus.UNREACHABLE:
            return {
                color: 'error',
                ...localizeStatus('unreachable', 'Destinatario irreperibile', 'Il destinatario non è reperibile', 'Il destinatario non è reperibile', { isMultiRecipient }),
            };
        case NotificationStatus.PAID:
            return {
                color: 'success',
                ...localizeStatus('paid', 'Pagata', 'Il destinatario ha pagato i costi della notifica', 'Il destinatario ha pagato i costi della notifica'),
            };
        case NotificationStatus.ACCEPTED:
            return {
                color: 'default',
                ...localizeStatus('accepted', 'Depositata', "L'ente ha depositato la notifica", "L'ente ha depositato la notifica"),
            };
        case NotificationStatus.EFFECTIVE_DATE:
            return {
                color: 'info',
                ...localizeStatus('effective-date', 'Perfezionata per decorrenza termini', 'Il destinatario non ha letto la notifica', 'Il destinatario non ha letto la notifica entro il termine stabilito', { isMultiRecipient }),
            };
        case NotificationStatus.VIEWED:
            if (statusObject?.recipient) {
                subject = getLocalizedOrDefaultLabel('notifications', `status.delegate`, `delegato ${statusObject.recipient}`, { name: statusObject.recipient });
            }
            return {
                color: 'success',
                ...localizeStatus('viewed', 'Avvenuto accesso', `Il ${subject} ha letto la notifica`, `Il ${subject} ha letto la notifica`, { subject, isMultiRecipient }),
            };
        case NotificationStatus.CANCELLED:
            return {
                color: 'warning',
                ...localizeStatus('canceled', 'Annullata', "L'ente ha annullato l'invio della notifica", "L'ente ha annullato l'invio della notifica"),
            };
        case NotificationStatus.CANCELLATION_IN_PROGRESS:
            return {
                color: 'warning',
                ...localizeStatus('cancellation-in-progress', 'Annullata', 'Annullamento in corso. Lo stato sarà aggiornato a breve.', 'Annullamento in corso. Lo stato sarà aggiornato a breve.'),
            };
        default:
            return {
                color: 'default',
...
function legalFactTypeForAnalogEvent(timelineStep, legalFactKey) {
    const attachments = timelineStep.details.attachments;
 ...
 */
export function getLegalFactLabel(timelineStep, legalFactType, legalFactKey) {
    const legalFactLabel = getLocalizedOrDefaultLabel('notifications', `detail.legalfact`, 'Attestazione opponibile a terzi');
    ...
    // -------------------------
    // Carlos Lombardi
    if (timelineStep.category === TimelineCategory.SEND_ANALOG_PROGRESS ||
        ...
        // eslint-disable-next-line functional/no-let
        let text = getLocalizedOrDefaultLabel('notifications', `detail.timeline.analog-workflow-attachment-kind.${type}`, '');
        if (text.length === 0) {
            text = getLocalizedOrDefaultLabel('notifications', 'detail.timeline.analog-workflow-attachment-kind.generic', `Documento allegato all'evento`);
        }
        return text;
        // Carlos Lombardi
        // PN-5484
    }
    else if (timelineStep.category === TimelineCategory.COMPLETELY_UNREACHABLE &&
        legalFactType === LegalFactType.ANALOG_FAILURE_DELIVERY) {
        return getLocalizedOrDefaultLabel('notifications', 'detail.timeline.legalfact.analog-failure-delivery', 'Deposito di avvenuta ricezione');
    }
    else if (timelineStep.category === TimelineCategory.ANALOG_FAILURE_WORKFLOW &&
        legalFactType === LegalFactType.AAR) {
        return getLocalizedOrDefaultLabel('notifications', 'detail.timeline.aar-document', 'Avviso di avvenuta ricezione');
    }
    else if (timelineStep.category === TimelineCategory.SEND_DIGITAL_PROGRESS &&
        legalFactType === LegalFactType.PEC_RECEIPT) {
        if (timelineStep.details.deliveryDetailCode === 'C001' ||
            timelineStep.details.deliveryDetailCode === 'DP00') {
            return `${receiptLabel} ${getLocalizedOrDefaultLabel('notifications', 'detail.timeline.legalfact.pec-receipt-accepted', 'di accettazione PEC')}`;
        }
        else if (timelineStep.details.deliveryDetailCode === 'C008' ||
            timelineStep.details.deliveryDetailCode === 'C010' ||
            timelineStep.details.deliveryDetailCode === 'DP10') {
            return `${receiptLabel} ${getLocalizedOrDefaultLabel('notifications', 'detail.timeline.legalfact.pec-receipt-not-accepted', 'di mancata accettazione PEC')}`;
        }
    }
    else if (timelineStep.category === TimelineCategory.SEND_DIGITAL_FEEDBACK &&
        legalFactType === LegalFactType.PEC_RECEIPT) {
        if (timelineStep.details.responseStatus === 'OK') {
            return `${receiptLabel} ${getLocalizedOrDefaultLabel('notifications', 'detail.timeline.legalfact.pec-receipt-delivered', 'di consegna PEC')}`;
        }
        else if (timelineStep.details.responseStatus === 'KO') {
            return `${receiptLabel} ${getLocalizedOrDefaultLabel('notifications', 'detail.timeline.legalfact.pec-receipt-not-delivered', 'di mancata consegna PEC')}`;
        }
        ...
        return `${legalFactLabel}: ${getLocalizedOrDefaultLabel('notifications', 'detail.timeline.legalfact.sender-ack', 'notifica presa in carico')}`;
    }
    else if (legalFactType === LegalFactType.DIGITAL_DELIVERY &&
        timelineStep.category === TimelineCategory.DIGITAL_SUCCESS_WORKFLOW) {
        return `${legalFactLabel}: ${getLocalizedOrDefaultLabel('notifications', 'detail.timeline.legalfact.digital-delivery-success', 'notifica digitale')}`;
    }
    else if (legalFactType === LegalFactType.DIGITAL_DELIVERY &&
        timelineStep.category === TimelineCategory.DIGITAL_FAILURE_WORKFLOW) {
        return `${legalFactLabel}: ${getLocalizedOrDefaultLabel('notifications', 'detail.timeline.legalfact.digital-delivery-failure', 'mancato recapito digitale')}`;
        // this is (at least in the examples I've seen)
        // related to the category NOTIFICATION_VIEWED
    }
    else if (legalFactType === LegalFactType.RECIPIENT_ACCESS) {
        return `${legalFactLabel}: ${getLocalizedOrDefaultLabel('notifications', 'detail.timeline.legalfact.recipient-access', 'avvenuto accesso')}`;
    }
 ...
 */
export function getNotificationTimelineStatusInfos(step, recipients, allStepsForThisStatus) {
    ...
    return TimelineStepFactory.createTimelineStep(step).getTimelineStepInfo({
        step,
        recipient,
        isMultiRecipient: recipients.filter((recDescription) => recDescription.denomination && recDescription.taxId && recDescription.recipientType).length > 1,
 ...
 */
function isInternalAppIoEvent(step) {
 ...
 */
function populateMacroStep(parsedNotification, timelineElement, status, acceptedStatusItems) {
    ...
    if (step) {
        ...
        else if (step.category === TimelineCategory.ANALOG_FAILURE_WORKFLOW &&
            step.details.generatedAarUrl) {
            status.steps.push({
                ...step,
                legalFactsIds: [
                    {
                        documentId: step.details.generatedAarUrl,
...
function fromLatestToEarliest(a, b) {
...
function populateMacroSteps(parsedNotification) {
    /* eslint-disable functional/no-let */
    let acceptedStatusItems = [];
    let deliveryMode;
    let deliveringStatus;
    ...
    for (const status of parsedNotification.notificationStatusHistory) {
        ...
        if (status.status === NotificationStatus.VIEWED) {
            const viewedSteps = status.steps.filter((s) => s.category === TimelineCategory.NOTIFICATION_VIEWED);
            if (viewedSteps.length) {
                ...
                if (mostOldViewedStep.details &&
                    mostOldViewedStep.details.delegateInfo) {
                    const { denomination, taxId } = mostOldViewedStep.details
                        .delegateInfo;
 ...
 */
const populateOtherDocuments = (notificationDetail) => {
    const timelineFiltered = notificationDetail.timeline.filter((t) => t.category === TimelineCategory.AAR_GENERATION);
    if (timelineFiltered.length > 0) {
        ...
        return timelineFiltered.map((t) => {
            ...
            const recipientData = isMultiRecipient && recIndex != null
                ...
                : '';
            const title = `${getLocalizedOrDefaultLabel('notifications', 'detail.timeline.aar-document', 'Avviso di avvenuta ricezione')}${recipientData}`;
            return {
                recIndex: t.details.recIndex,
                documentId: t.details.generatedAarUrl,
                documentType: LegalFactType.AAR,
                title,
                digests: {
                    sha256: '',
...
export const getF24Payments = (payments, recIndex, onlyF24 = true) => payments.reduce((arr, payment, index) => {
...
export const getPagoPaF24Payments = (payments, recIndex, withLoading = false) => payments.reduce((arr, payment, index) => {
    if (payment.pagoPa) {
        // eslint-disable-next-line functional/immutable-data
        arr.push({
            pagoPa: {
 ...
 */
export const populatePaymentsPagoPaF24 = (timeline, pagoPaF24Payemnts, checkoutPayments) => {
    ...
    for (const userPayment of pagoPaF24Payemnts) {
        ...
        if (checkoutPayments.length &&
            checkoutPayments?.findIndex((payment) => payment.creditorTaxId === userPayment.pagoPa?.creditorTaxId &&
                payment.noticeCode === userPayment.pagoPa?.noticeCode) === -1) {
            continue;
        }
        // 3. Get payment by creditorTaxId and noticeCode from checkout
        const checkoutPayment = checkoutPayments.find((p) => p.creditorTaxId === userPayment?.pagoPa?.creditorTaxId &&
            p.noticeCode === userPayment?.pagoPa?.noticeCode);
        const timelineEvent = paymentTimelineStep.find((item) => {
            const paymentDetails = item.details;
            return (paymentDetails.creditorTaxId === userPayment?.pagoPa?.creditorTaxId &&
                paymentDetails.noticeCode === userPayment?.pagoPa?.noticeCode);
        })?.details;
        if (timelineEvent) {
            Object.keys(timelineEvent).forEach((key) => (timelineEvent[key] === undefined ? delete timelineEvent[key] : {}));
        }
        const pagoPAPayment = {
            ...userPayment.pagoPa,
            ...checkoutPayment,
...
function timelineElementMustBeShown(t) {
    if (t.category === TimelineCategory.SEND_ANALOG_PROGRESS ||
        ...
        const deliveryDetailCode = t.details.deliveryDetailCode;
 ...
 * @param  {NotificationDetail} notificationDetail
 */
const insertCancelledStatusInTimeline = (notificationDetail) => {
    const timelineCancelledElement = notificationDetail.timeline.find((el) => el.category === TimelineCategory.NOTIFICATION_CANCELLED);
    if (!timelineCancelledElement) {
        const timelineCancellationRequestElement = notificationDetail.timeline.find((el) => el.category === TimelineCategory.NOTIFICATION_CANCELLATION_REQUEST);
...
export function parseNotificationDetail(notificationDetail) {
