 ...
 */
export function basicInitialUserData(yupMatcher, noLoggedUserData) {
...
export function adaptedTokenExchangeError(originalError) {
    ...
    if (originalError.response?.status === 403) {
        return {
            ...
            response: {
                ...
                customMessage: {
                    title: getLocalizedOrDefaultLabel('common', `messages.lacking-grants-for-app-title`, 'Non sei autorizzato ad accedere ...'),
                    message: getLocalizedOrDefaultLabel('common', `leaving-app.title`, 'Stai uscendo da SEND ...'),
                },
            },
        };
        ...
        return {
            ...
            response: {
                ...originalError.response,
                customMessage: {
                    title: getLocalizedOrDefaultLabel('common', `messages.451-title`, 'Piattaforma non accessibile'),
                    message: getLocalizedOrDefaultLabel('common', `messages.451-message`, 'Non è possibile accedere alla piattaforma'),
                },
            },
        };
    }
    ...
    return {
        ...
        response: {
            ...originalError.response,
            customMessage: {
                title: getLocalizedOrDefaultLabel('common', `messages.generic-token-exchange-problem`, "Non è stato possibile completare l'accesso ..."),
                message: getLocalizedOrDefaultLabel('common', `leaving-app.title`, 'Stai uscendo da SEND ...'),
            },
        },
    };
    ...
    for (const [key, value] of Object.entries(obj)) {
        if (value !== null) {
            if (typeof value === 'object' && !Array.isArray(value)) {
                ...
                // eslint-disable-next-line functional/immutable-data
                newObj[key] = value.map((item) => typeof item === 'object' ? removeNullProperties(item) : item); // add type assertion here
            }
            else {
                // eslint-disable-next-line functional/immutable-data
                newObj[key] = value;
