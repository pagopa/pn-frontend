...
var GENERAL_STEPS;
(function (GENERAL_STEPS) {
    GENERAL_STEPS["NOT_STARTED"] = "NotStarted";
 ...
 */
export function useProcess(sequence) {
    const fullSequence = useMemo(() => [GENERAL_STEPS.NOT_STARTED, ...sequence], [sequence]);
    const [currentSituation, setCurrentSituation] = useState({
        step: GENERAL_STEPS.NOT_STARTED,
        isActive: false,
    });
    const mustProceedToStep = useCallback((step) => fullSequence.indexOf(currentSituation.step) === fullSequence.indexOf(step) - 1 &&
        !currentSituation.isActive, [currentSituation]);
    const isFinished = useCallback(() => fullSequence.indexOf(currentSituation.step) === fullSequence.length - 1 &&
        !currentSituation.isActive, [currentSituation]);
    ...
    const performStep = useCallback(async (step, action) => {
