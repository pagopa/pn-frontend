import { DowntimeStatus, KnownFunctionality, isKnownFunctionality, } from '../../models';
...
/* ------------------------------------------------------------------------
   the API
   ------------------------------------------------------------------------ */
export function createAppStatusApi(apiClientProvider) {
    return {
        getCurrentStatus: async () => {
            const realApiResponse = await apiClientProvider().get(DOWNTIME_STATUS());
        ...
        getDowntimeLogPage: async (params) => {
            const realApiResponse = await apiClientProvider().get(DOWNTIME_HISTORY(params));
            const apiResponse = realApiResponse.data;
            // pn-validator validation
        ...
        getLegalFactDetails: async (legalFactId) => {
            const realApiResponse = await apiClientProvider().get(DOWNTIME_LEGAL_FACT_DETAILS(legalFactId));
...
/* ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ */
function beDowntimeToFeDowntime(downtime) {
    ...
    if (isKnownFunctionality(downtime.functionality)) {
        ...
        result.knownFunctionality = downtime.functionality;
    }
...
function beDowntimeStatusToFunctionalityCurrentStatus(functionality, appStatusDTO) {
    const currentIncident = appStatusDTO.openIncidents.find((downtime) => downtime.functionality === functionality);
    if (currentIncident) {
        return {
            rawFunctionality: functionality,
...
function unknownDowntimeToFunctionalityCurrentStatus(downtime) {
    return {
        rawFunctionality: downtime.functionality,
        isOperative: false,
...
function beAppStatusToFeAppStatus(appStatusDTO) {
...
function beDowntimeLogPageToFeDowntimeLogPage(downtimeLogPageDTO) {
    /* eslint-disable functional/immutable-data */
    const result = {
