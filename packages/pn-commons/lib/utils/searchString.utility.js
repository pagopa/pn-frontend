 ...
 */
export function searchStringCleanDenomination(wholeSearchString) {
 ...
 */
export function searchStringLimitReachedText(searchString, maxLength = 80) {
    return searchString.length > maxLength
        ? ` (${getLocalizedOrDefaultLabel('common', 'validation.search-pattern-length-limit', 'max XXX caratteri', { maxLength })})`
        : '';
}
/**
...
export function useSearchStringChangeInput(maxLength = 80) {
    ...
    return useCallback((newInputValue, setNewValue) => {
        ...
        if (cleanedValue.length < newInputValue.length) {
            const message = getLocalizedOrDefaultLabel('common', 'validation.invalid-characters-not-inserted', 'caratteri invalidi non inseriti');
