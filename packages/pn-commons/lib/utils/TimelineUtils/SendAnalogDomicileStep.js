import { PhysicalCommunicationType } from '../../types';
...
export class SendAnalogDomicileStep extends TimelineStep {
    getTimelineStepInfo(payload) {
        ...
        if (payload.step.details.serviceLevel ===
            PhysicalCommunicationType.REGISTERED_LETTER_890) {
            return {
                ...this.localizeTimelineStatus('send-analog-domicile-890', payload.isMultiRecipient, 'Invio via raccomandata 890', `È in corso l'invio della notifica a ${payload.recipient?.denomination} all'indirizzo ${payload.step.details.physicalAddress?.address} tramite raccomandata 890.`, {
        ...
        return {
            ...this.localizeTimelineStatus('send-analog-domicile-ar', payload.isMultiRecipient, 'Invio via raccomandata A/R', `È in corso l'invio della notifica a ${payload.recipient?.denomination} all'indirizzo ${payload.step.details.physicalAddress?.address} tramite raccomandata A/R.`, {
                ...this.nameAndTaxId(payload),
                ...this.completePhysicalAddress(payload.step),
            }),
        };
