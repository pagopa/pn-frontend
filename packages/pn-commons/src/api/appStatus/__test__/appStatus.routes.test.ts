...
describe('AppStatus routes', () => {
  ...
  it('should compile DOWNTIME_HISTORY with all parameters set', () => {
    expect(
      ...
      `/downtime/v1/history?fromTime=${encodeURIComponent(
        '2022-10-18T09:34:51Z'
      )}&toTime=${encodeURIComponent(
        '2022-10-20T09:34:51Z'
      )}&functionality=NOTIFICATION_WORKFLOW&page=pageId&size=28`
    );
  });
  ...
  it('should compile DOWNTIME_HISTORY with two functionalities', () => {
    expect(
      ...
      `/downtime/v1/history?fromTime=${encodeURIComponent(
        '2022-10-18T09:34:51Z'
      )}&functionality=NOTIFICATION_WORKFLOW&functionality=NOTIFICATION_CREATE&size=54`
    );
  });

  it('should compile DOWNTIME_LEGAL_FACT_DETAILS for a given legalFactId', () => {
    expect(DOWNTIME_LEGAL_FACT_DETAILS('some-legal-fact')).toEqual(
      '/downtime/v1/legal-facts/some-legal-fact'
    );
  });
});
