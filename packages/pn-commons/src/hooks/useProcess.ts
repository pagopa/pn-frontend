...
export function useProcess<T>(sequence: Array<T>) {
  ...
  const fullSequence: Array<STEP_TYPE> = useMemo(() => [GENERAL_STEPS.NOT_STARTED,...sequence], [sequence]);

  const [currentSituation, setCurrentSituation] = useState<{ step: STEP_TYPE; isActive: boolean}>
    ({ step: GENERAL_STEPS.NOT_STARTED , isActive: false });

  /* eslint-disable-next-line arrow-body-style */
  const mustProceedToStep = useCallback((step: STEP_TYPE) => {
    return fullSequence.indexOf(currentSituation.step) === fullSequence.indexOf(step) - 1
      && !currentSituation.isActive;
  }, [currentSituation]);

  /* eslint-disable-next-line arrow-body-style */
  const isFinished = useCallback(() => {
    return fullSequence.indexOf(currentSituation.step) === fullSequence.length - 1
      && !currentSituation.isActive;
  }, [currentSituation]);
