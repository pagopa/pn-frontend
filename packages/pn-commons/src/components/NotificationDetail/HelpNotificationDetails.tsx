...
const HelpNotificationDetails: React.FC<HelpNotificationDetailsProps> = ({
  ...
  const [validatedContactChannels, setValidatedContactChannels] =
    useState<ValidatedContactChannels>({
      phoneNumber: null,
      mail: null,
      website: null,
    });
  ...
  useEffect(() => {
    const fetchValidatedContactChannels = async () => {
      const validatedPhoneNumber = (await yup
        .string()
        .matches(dataRegex.phoneNumber)
        .isValid(phoneNumber))
        ? phoneNumber
        : null;
      const validatedMail = (await yup.string().matches(dataRegex.email).isValid(mail))
        ? mail
        : null;
  ...
  return (
    <>
        ...
        <Grid container direction="row" alignItems="center" mt={2}>
          {validatedContactChannels.phoneNumber && (
            <ButtonNaked
              ...
              href={`tel:${validatedContactChannels.phoneNumber}`}
            >
              {validatedContactChannels.phoneNumber}
            </ButtonNaked>
          )}
          ...
          {validatedContactChannels.website && (
            <ButtonNaked
              ...
              startIcon={<WebIcon />}
              sx={{ ml: 2 }}
              href={validatedContactChannels.website}
            >
      ...
      {!someContactChannelPresent && (
        <Typography variant="body1">(non ci sono trovati dati di contatto)</Typography>
      )}
      <Divider sx={{ mt: 2 }} />
