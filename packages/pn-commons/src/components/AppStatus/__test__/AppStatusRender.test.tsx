...
jest.mock('../MobileDowntimeLog', () => () => <div data-testid="mock-mobile-downtime-log">Mobile downtime log</div>);

jest.mock('../DesktopDowntimeLog', () => () => <div data-testid="mock-desktop-downtime-log">Desktop downtime log</div>);

jest.mock('../../EmptyState', () => (props: any) => <div data-testid="mock-empty-state">{String(props.sentimentIcon)}</div>);

const mockActionIds = { GET_CURRENT_STATUS: 'mock-get-current-status', GET_DOWNTIME_LOG_PAGE: 'mock-get-downtime-log-page' };

const mockAppStatus: AppStatusData = {
  pagination: {page: 0, size: 10, resultPages: ["0"]},
  currentStatus: {appIsFullyOperative: true, lastCheckTimestamp: "2022-12-28T15:43:19.190Z", statusByFunctionality: []},
  downtimeLogPage: {downtimes: [
    {
...
describe('AppStatusRender component', () => {
  ...
  it('empty downtime list', async () => {
    ...
    await act(async () => void render(
      <AppStatusRender 
        actionIds={mockActionIds} appStatus={{...mockAppStatus, downtimeLogPage: { downtimes: [] }}} clearLegalFactDocument={() => {}} 
        clearPagination={() => {}} fetchCurrentStatus={() => {}} fetchDowntimeLegalFactDocumentDetails={() => { return undefined; }}
        fetchDowntimeLogPage={() => {}} setPagination={() => {}}
      />
    ));
    ...
    const satisfied = emptyStateComponent && within(emptyStateComponent).queryByText(String(KnownSentiment.SATISFIED));
    const dissatisfied = emptyStateComponent && within(emptyStateComponent).queryByText(String(KnownSentiment.DISSATISFIED));
  ...
  it('error in status API', async () => {
    ...
    await act(async () => void render(
      <AppStatusRender 
        ...
        fetchDowntimeLogPage={() => {}} setPagination={() => {}}
      />,
      { preloadedState: {appState: apiOutcomeTestHelper.appStateWithMessageForAction(mockActionIds.GET_CURRENT_STATUS)}}
    ));
    ...
    const mobileDonwtimeLogComponent = screen.queryByTestId('mock-mobile-downtime-log');
    const emptyStateComponent = screen.queryByTestId('mock-empty-state');
    const errorStatusComponent = screen.queryByTestId(`api-error-${mockActionIds.GET_CURRENT_STATUS}`);
    const errorDowntimeComponent = screen.queryByTestId(`api-error-${mockActionIds.GET_DOWNTIME_LOG_PAGE}`);
  ...
  it('error in both status and downtime API', async () => {
    ...
    const mockAppState = apiOutcomeTestHelper.appStateWithMessageForAction(mockActionIds.GET_CURRENT_STATUS);
    mockAppState.messages.errors.push(apiOutcomeTestHelper.errorMessageForAction(mockActionIds.GET_DOWNTIME_LOG_PAGE));
