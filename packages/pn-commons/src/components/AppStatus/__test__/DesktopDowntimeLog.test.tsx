...
describe('DesktopDowntimeLog component - with data', () => {
  ...
  beforeEach(async () => {
    ...
    await act(async () => void render(
      <DesktopDowntimeLog downtimeLog={exampleDowntimeLogPage} getDowntimeLegalFactDocumentDetails={getLegalFactDetailsMock} />
    ));
  ...
  it("date values", async () => {
    ...
    const startDateComponent1 = within(firstRowStartDate).queryByText( new RegExp(formatDate(incidentTimestamps[4])) );
    const startHourComponent1 = within(firstRowStartDate).queryByText( new RegExp(formatTimeHHMM(incidentTimestamps[4])) );
    ...
    const endDateComponent3 = within(thirdRowEndDate).queryByText( new RegExp(formatDate(incidentTimestamps[1])) );
    const endHourComponent3 = within(thirdRowEndDate).queryByText( new RegExp(formatTimeHHMM(incidentTimestamps[1])) );
  ...
  it('functionality values', async () => {
    const functionalityIndex = Array.from(headerRowColumns).findIndex(elem => within(elem).queryByText("downtimeList.columnHeader.functionality"));

    // first row - known functionality
    const firstDataRow = rowComponents[1];
    const firstRowColumns = within(firstDataRow).queryAllByRole("cell");
    const firstRowFunctionality = firstRowColumns[functionalityIndex];
    const description1 = within(firstRowFunctionality).queryByText(`legends.knownFunctionality.${KnownFunctionality.NotificationWorkflow}`);
    ...
    const description2 = within(secondRowFunctionality).queryByText(new RegExp("legends.unknownFunctionality"));
    ...
    const description3 = within(thirdRowFunctionality).queryByText(`legends.knownFunctionality.${KnownFunctionality.NotificationCreate}`);
    expect(description3).toBeInTheDocument();
  });

  it('legalFactId values', async () => {
    const legalFactIndex = Array.from(headerRowColumns).findIndex(elem => within(elem).queryByText("downtimeList.columnHeader.legalFactId"));
    ...
    const description1 = within(firstRowLegalFact).queryByText(`legends.noFileAvailableByStatus.${DowntimeStatus.KO}`);
    expect(description1).toBeInTheDocument();
  ...
  it('status values', async () => {
    const statusIndex = Array.from(headerRowColumns).findIndex(elem => within(elem).queryByText("downtimeList.columnHeader.status"));
