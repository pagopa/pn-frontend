...
describe('DesktopDowntimeLog component - with data', () => {
  ...
  beforeEach(async () => {
    ...
    await act(async () => void render(
      <MobileDowntimeLog downtimeLog={exampleDowntimeLogPage} getDowntimeLegalFactDocumentDetails={getLegalFactDetailsMock} />
    ));
  ...
  it("headers - in first card", async () => {
    ...
    const startDateIndex = Array.from(labelComponents).findIndex(elem => within(elem).queryByText("downtimeList.columnHeader.startDate"));
    const endDateIndex = Array.from(labelComponents).findIndex(elem => within(elem).queryByText("downtimeList.columnHeader.endDate"));
    ...
    const legalFactIdIndex = Array.from(labelComponents).findIndex(elem => within(elem).queryByText("downtimeList.columnHeader.legalFactId"));
    const functionalityIndex = Array.from(labelComponents).findIndex(elem => within(elem).queryByText("downtimeList.columnHeader.functionality"));
    const statusIndex = Array.from(labelComponents).findIndex(elem => within(elem).queryByText("downtimeList.columnHeader.status"));
  ...
  it("date values", async () => {
    ...
    const startDateComponent1 = within(firstCardStartDate).queryByText( new RegExp(formatDate(incidentTimestamps[4])) );
    const startHourComponent1 = within(firstCardStartDate).queryByText( new RegExp(formatTimeHHMM(incidentTimestamps[4])) );
    ...
    const endDateComponent3 = within(thirdCardEndDate).queryByText( new RegExp(formatDate(incidentTimestamps[1])) );
    const endHourComponent3 = within(thirdCardEndDate).queryByText( new RegExp(formatTimeHHMM(incidentTimestamps[1])) );
  ...
  it('functionality values', async () => {
    ...
    const description1 = within(firstCardFunctionality).queryByText(`legends.knownFunctionality.${KnownFunctionality.NotificationWorkflow}`);
    ...
    const description2 = within(secondCardFunctionality).queryByText(new RegExp("legends.unknownFunctionality"));
    ...
    const description3 = within(thirdCardFunctionality).queryByText(`legends.knownFunctionality.${KnownFunctionality.NotificationCreate}`);
  ...
  it('legalFactId values', async () => {
    ...
    const description1 = within(firstCardLegalFactId).queryByText(`legends.noFileAvailableByStatus.${DowntimeStatus.KO}`);
    expect(description1).toBeInTheDocument();
    ...
    const description2 = within(secondCardLegalFactId).queryByText(`legends.noFileAvailableByStatus.${DowntimeStatus.OK}`);
    expect(description2).toBeInTheDocument();
