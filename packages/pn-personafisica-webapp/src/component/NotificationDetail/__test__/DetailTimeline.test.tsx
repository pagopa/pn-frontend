  ...
  it('renders detail timeline', async () => {
    ...
    for (const item of timelineItems!) {
      ...
      expect(dateItems[0]).toHaveTextContent(
        getMonthString(notificationToFe.timeline[timelineIndex].timestamp)
      );
      expect(dateItems[1]).toHaveTextContent(
        getDay(notificationToFe.timeline[timelineIndex].timestamp)
      );
      expect(dateItems[2]).toHaveTextContent(
        getTime(notificationToFe.timeline[timelineIndex].timestamp)
      );
