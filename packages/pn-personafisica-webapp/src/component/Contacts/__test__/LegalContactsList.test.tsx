...
describe('LegalContactsList Component', () => {
  ...
  it('renders component - validating pec', async () => {
    // render component
    await act(async () => {
      result = render(
        <DigitalContactsCodeVerificationProvider>
    ...
    const cancelValidation = result?.getByTestId('cancelValidation');
    fireEvent.click(cancelValidation!);
    const dialog = await waitFor(() => result?.getByTestId('cancelVerificationModal'));
  ...
  it('edits pec', async () => {
    ...
    await waitFor(() => {
      expect(form!).not.toHaveTextContent(defaultAddress!.value);
    });
