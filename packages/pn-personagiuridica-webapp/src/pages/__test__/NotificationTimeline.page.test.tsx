...
const getLegalFactIds = (notification: NotificationDetailModel, recIndex: number) => {
  // the first legacy-timeline event carrying a legal fact, for the given recipient: this is the
  // one behind the first "download-legalfact" button actually rendered on the page
  const timelineElementWithLegalFact = notification.timeline.filter(
    (t) =>
      t.category === TimelineCategory.SEND_ANALOG_PROGRESS &&
      t.details.recIndex === recIndex &&
      t.legalFactsIds?.length
  )[0];
  return timelineElementWithLegalFact.legalFactsIds![0];
};
...
vi.mock('../../services/configuration.service', async () => {
  return {
    ...
    getConfiguration: () => ({
      ...Configuration.get<PgConfiguration>(),
