...
const AppStatus = () => {
  ...
  const legalFactDocumentDetails = useAppSelector(
    (state: RootState) => state.appStatus.legalFactDocumentData
  );
  ...
  return (
    <Box p={3}>
      <Stack direction="column">
        {/* Titolo status */}
        <TitleBox
          title={t('appStatus.title')}
          variantTitle="h4"
          subTitle={t('appStatus.subtitle')}
          variantSubTitle="body1"
        />

        {/* Dati relativi al status */}
        <ApiErrorWrapper
          apiId={APP_STATUS_ACTIONS.GET_CURRENT_STATUS}
        ...
        <Typography variant="h6" sx={{ mt: '36px', mb: 2 }}>
          {t('downtimeList.title')}
        </Typography>

        {/* Dati relativi al elenco di downtime */}
        <ApiErrorWrapper
          apiId={APP_STATUS_ACTIONS.GET_DOWNTIME_LOG_PAGE}
        ...
        >
          {downtimeLog && downtimeLog.downtimes.length > 0 ? (
            ...
            <EmptyState
              ...
              emptyMessage={t('downtimeList.emptyMessage')}
            />
          )}
          {downtimeLog && downtimeLog.downtimes.length > 0 && (
            <CustomPagination
