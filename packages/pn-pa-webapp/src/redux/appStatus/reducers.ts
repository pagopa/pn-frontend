...
import { getCurrentAppStatus, getDowntimeHistory } from './actions';
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getDowntimeHistory.fulfilled, (state, action) => {
      ...
      if (
        ...
        !state.pagination.resultPages.includes(action.payload.nextPage)
      ) {
...
export const { clearPagination, setPagination } = appStatusSlice.actions;
